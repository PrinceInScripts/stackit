...
export default function HomePage() {
  ...
  const filtered = questions
    ...
    .filter(q => sortBy === 'unanswered' ? q.isAnswered === false : true)
  ...
  return (
    <div className="space-y-6">
      {/* Header: Ask + Filters + Search */}
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <Link to="/ask" className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
          Ask New Question
        </Link>
        ...
        <div className="flex gap-3">
          ...
          <select className="border rounded-lg px-3 py-1 w-24">
            <option>More</option>
          </select>
      ...
      <ul className="space-y-4">
        {paginated.map(q => (
          <li key={q._id} className="border rounded-lg p-4 hover:shadow">
            <div className="flex justify-between items-center">
              <Link to={`/questions/${q._id}`} className="text-xl font-semibold hover:underline">
                {q.title}
              </Link>
            ...
            <p className="mt-2 text-gray-700 line-clamp-3">{q.description.replace(/<[^>]+>/g, '')}</p>
